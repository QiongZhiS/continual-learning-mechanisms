...
**Transfer-distance stratification**: results are reported separately for near transfer (A→B) and far transfer (B→C / C→D). **Primary criterion: in far-transfer domains, the strong-prior cells (② or ①+②) significantly below weak-0 (c1) = direct evidence for the strong-prior liability** (and thus for the updateable-prior corollary). Secondary reading: far-domain ②<① (structural bias below generic pretraining) is the operationalization of "over-generalization" — a strong prior that forces the regularities of context A onto context B is negative transfer; in a drifting world, a strong prior is a structural liability: old priors process new worlds, and the lag is structural, not a matter of effort.
