...
**现状（2026-08-11）**：M0 底座复现完成；M1（探索机制）主判据在符号域失败——推理期探索对"解唯一但模型未学会"的问题结构性无效（负结果+机制定位已完成，重设计方向已确定，见 §6-E1）。实验按失败分支推进中。
...
带后缀的编号（E0b/E0d/E2b/E6a/E7b 等）为同一实验的变体臂（扰动扫描/结构生长/衰减曲线/序列化三臂/免疫中和）。里程碑与实验的映射见 §5。
...
**第一张骨牌判断**：训练/推理分离的拆除。理由：可微-离散混合架构已存在；控制论提供在线更新的现成数学；QLoRA/夜间微调已工程可行——E3-D 即第一块砖。
...
**脑科学交叉**：MIT 失语症研究（推理不需语言区）+ 逆向模型研究（噪声+逆向模型=结构化探索）+ 幽灵点（学习是分岔现象）+ 睡眠相变（去稳定化-重新稳定）——全部指向同一结论：**智能的底层是动力学 + 结构化探索 + 双模记忆 + 临界切换，语言和动作是后来的乘客。**
...
| 推论 | 内容 | 对应实验 | 前沿锚点 |
|------|------|---------|---------|
| 推论 1 | 并行探索 + 质量选择在**等计算预算对照**下仍显著优于单路径确定性推理 | E1 | PTRM：K=100 时 62.6%→91.2% |
...
执行顺序：E0 → E1 → E2/E4 并行 → E3（最难）→ E5 → E6。（E8 候选机制 / E9 自我认知轨道为独立轨道，不占关键路径。）
...
- **硬件**：单张消费级 GPU（RTX 4060 8GB），实测训练速率 ~5.6k 步/小时（batch 128）
...
本环境实测（RTX 4060 8GB · TRM 5M · batch 128 · 域 A）：训练 ~5.6k 步/小时（2.3 万步 = 4.2h → 33% 精确率）；eval 200 谜题：K=1 约 9s / K=100 约 44min。

| 实验 | 运行数（粗估） | 单运行 | 种子 | 合计（粗估） |
|------|--------------|--------|------|------------|
| E1 域 B（主判据） | 5+ | 4-24h | 5 | 5-10 天 |
| E2/E4 | 5-10 | 4-24h | 5 | 10-20 天 |
| E3（蒸馏核心） | 5-15 | 4-48h | 5 | 15-40 天 |
| E5/E6a（三臂 × 7 域） | 20-30 | 4-48h | 3 | 30-80 天 |

E6a 预算已冻结：种子数 5→3（保留 7 域）——依据：M0 阶段实测效应量巨大（Cohen's d = 11.5），低功效需求下 3 种子足够防单种子巧合；7 域保留证据面。兜底：后段若方差超预期可回补种子（域不变）。
...
**失败轨迹共享（"死亡回归"的修正版）**：仅并行展开 + 选择器 = 暴力并行搜索 + 样本选择——100 条轨迹独立演化，第 50 条发现的捷径不会回传给第 1-49 条。**完整版 = 并行展开 + 跨展开失败轨迹共享**：每条展开的失败轨迹簇（撞过的墙）写入共享禁忌索引，后续展开优先避开——失败信息异步回传后，才构成"带着死因重来"的认知迭代。禁忌索引与 E3 负向蒸馏复用同一组件（单一开关因子，防重复写入/权重叠加）。

**M1 结果与失败分支（2026-08-11 · 当前状态）**：

域 B（RPN 栈机算术）从零训 4000 步后实测：K=1 深扩展基线 0.478 / K=10 0.476 / K=100 0.476——**K 增大零增益**，E1 主判据失败，推论 1 在符号域不支持。失败机制经三轮判别完整定位：
...
- **逆向模型（GCML 式目标导向探索）**：纯噪声探索在"无盆"平坦地形上无效——噪声没有方向。设计：增量训练轻量逆向模型（~2M 参数），输入"当前状态 + 目标"→ 输出"朝目标的差分方向向量"；推理时该方向 + 高斯噪声 = 有指向的展开。可证伪预测：域 B 上"逆向指引+噪声"的 K 增益恢复（vs 纯噪声零增益 + "仅逆向无噪声"退化到贪心）。成本约 2h 增量训练
...
| 里程碑 | 内容 | 判据 | 周期 |
|--------|------|------|------|
| M0 | 底座复现（域 A） | 并行展开增益复现 + 吸引盆可视化 | ✅ 完成（拆子：M0.1 基线 / M0.2 可视化 / M0.3 可行性验证 / M0.4 域序列生成器） |
| M1 | E1 探索机制（域 A→B） | K 扩展在符号域成立 | 🔴 **已触发失败分支**（K 零增益，机制定位完成，重设计推进中） |
| M2 | E2 记忆 + E4 组合（域 B→C） | 保持率 + 鸭嘴兽测试通过 | 待执行 |
| M3 | E3 蒸馏（核心） | T(n) 下降？**最大未知** | 待执行 |
| M4 | E5 因果 + E6a/E6b + E7 | E6a 指纹判据（H1）+ 归因路径 | 待执行 |
| M5 | 真实域放大（长期轨道） | 最优配置迁移到文本/工具域 | 弹性 |
...
| M1 K 扩展在符号域失效 | 探索机制只对谜题域有效 | ✅ 已触发并完成机制定位 → 动力学调试 / 逆向模型方向 |
