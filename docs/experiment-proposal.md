...
**现状**：M0 底座复现完成；M1（探索机制）主判据在符号域失败——推理期探索对"解唯一但模型未学会"的问题结构性无效（负结果与机制定位见 §6-E1）。
...
| 里程碑 | 内容 | 判据 | 状态 |
|--------|------|------|------|
| M0 | 底座复现（域 A） | 并行展开增益复现 + 吸引盆可视化 | ✅ 完成 |
| M1 | E1 探索机制（域 A→B） | K 扩展在符号域成立 | 🔴 已触发失败分支（机制定位完成，见 §6-E1） |
